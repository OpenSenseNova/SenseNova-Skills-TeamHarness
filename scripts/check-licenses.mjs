...
// Optional native binaries differ by runner OS/CPU. Their parent packages are
// audited below; omitting these platform-specific variants keeps the report
// stable on developer machines and in Linux CI.
const PLATFORM_PACKAGE_PATTERN = /-(?:aix|android|darwin|freebsd|linux|openbsd|sunos|win32)-(?:arm64|armv7|ia32|ppc64|riscv64|x64)(?:-|$)/u;
...
function visit(node, fallbackName = '', cwd = root) {
  ...
  if (
    ...
    && name !== '@ai-native-collaboration/local-computer'
    && !PLATFORM_PACKAGE_PATTERN.test(name)
  ) {
...
const reportWithAssetNote = report.replace(
  'No external image, font, audio, video, or presentation fixture is distributed by the release package.',
  'The architecture illustration at `assets/sensenova-team-harness-architecture.png` is generated for this project with OpenAI image generation; no external image assets were supplied.',
);

if (process.argv.includes('--write')) {
  writeFileSync(reportPath, reportWithAssetNote, 'utf8');
  process.stdout.write(`Wrote ${lines.length} production dependency notices.\n`);
} else {
  const current = readFileSync(reportPath, 'utf8');
  if (current !== reportWithAssetNote) throw new Error('THIRD_PARTY_NOTICES.md is stale; run node scripts/check-licenses.mjs --write.');
