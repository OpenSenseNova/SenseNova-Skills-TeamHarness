...
export class WorkspaceService {
  ...
  private renderDeveloperInstructions(agentId: string): string {
    ...
    return [
      ...
      '### Resources',
      '',
      '- Project Resources are the input files a Human uploaded for the Project (CSV, Markdown, and other source material). They are distinct from Artifacts, which are the outputs you publish.',
      '- `teamctl resource list` lists every active Resource in the current Project you can access — each entry has its `resourceId`, `name`, `path`, `kind` (`file` or `directory`), `mediaType`, and `byteLength`.',
      '- `teamctl resource read <resource-id>` fetches one file Resource into the Agent work directory and returns its `filePath`; open that file to read the content. Directories have no readable content.',
      '- When a task refers to Project data, files, or "资源/项目资源", run `teamctl resource list` first to discover the source files, then `teamctl resource read <resource-id>` for each one you need before producing a result.',
      '',
      '### Artifacts',
      '',
