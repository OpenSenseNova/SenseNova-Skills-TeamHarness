...
export const runtimeOptions: Array<{ value: RuntimeId; label: string; description: string }> = [
  ...
  { value: 'hermes', label: 'Hermes Agent', description: '使用这台计算机上已安装的 Hermes ACP Runtime' },
];
...
export function computerLoadErrorMessage(error: unknown): string {
  if (error instanceof ApiError && error.status === 404) {
    return '当前运行的服务版本过旧，无法读取计算机。请重启后端服务后再试。';
  }
...
export function AgentRuntimeFields({ computers, children, onSetupComputer }: {
  ...
  const selectedModelCapability = runtimeConfiguration?.models.find((model) => model.id === selectedModel);
  const reasoningOptions = (runtimeConfiguration?.reasoningEfforts ?? []).filter((effort) => (
    selectedModelCapability?.supportedReasoningEfforts === null
    || selectedModelCapability === undefined
    || selectedModelCapability.supportedReasoningEfforts.includes(effort.id as Exclude<RuntimeReasoningEffort, null>)
  ));
    ...
    <>
      {!bindableComputers.length && (
        <Alert
          ...
          description={activeComputers.length
            ? '可以启动已有计算机，或添加另一台已安装并登录运行时的计算机。'
            : '先添加这台计算机，再连接本机已经安装并登录的运行时。'}
          ...
          optionRender={(option) => {
            ...
            return (
              <Space style={{ width: '100%', justifyContent: 'space-between' }}>
                ...
                {computer && !computer.runtimes.some((runtime) => runtime.availability === 'ready') && <Text type="secondary">没有可用的运行时</Text>}
              </Space>
            );
          }}
        />
      ...
      {selectedComputer && !readyRuntimes.length && (
        <Alert
          ...
          showIcon
          title="这台计算机没有可用的运行时"
          description="请先在这台计算机上安装并登录对应的命令行工具。"
          style={{ marginBottom: 18 }}
        />
      )}
      <Form.Item name="runtimeId" label="运行时" rules={[{ required: true, message: '请选择运行时' }]}>
        <Select
          size="large"
          placeholder="选择运行时"
      ...
      {selectedRuntime?.availability === 'ready' && !runtimeConfiguration && (
        <Alert
          ...
          showIcon
          title="运行时能力尚未完成检测"
          description="本地计算机需要重新上报 ACP 模型、推理强度和模式后才能绑定。"
          style={{ marginBottom: 18 }}
        />
      )}
      <Form.Item
        ...
        extra={runtimeConfiguration?.defaultModelId
          ? `留空时使用 Runtime 当前默认模型：${runtimeConfiguration.defaultModelId}`
          : '留空时使用 Runtime 当前默认模型。'}
      >
        <Select
          size="large"
          allowClear
          disabled={!runtimeConfiguration?.models.length}
          placeholder={runtimeConfiguration?.models.length ? '使用 Runtime 默认模型' : 'Runtime 未暴露模型选择'}
      ...
      <Form.Item
        ...
        label="推理强度"
        extra="Runtime 不支持所选强度时，Attempt 会明确失败，不会静默退回默认值。"
      >
        <Select
          size="large"
          allowClear
          disabled={!reasoningOptions.length}
          placeholder={reasoningOptions.length ? '使用 Runtime 默认值' : 'Runtime 未暴露推理强度'}
      ...
      <Form.Item
        ...
        extra={runtimeConfiguration?.defaultModeId
          ? `留空时使用 Runtime 当前默认模式：${runtimeConfiguration.defaultModeId}`
          : '留空时使用 Runtime 当前默认模式。'}
      >
        <Select
          size="large"
          allowClear
          disabled={!runtimeConfiguration?.modes.length}
          placeholder={runtimeConfiguration?.modes.length ? '使用 Runtime 默认模式' : 'Runtime 未暴露模式选择'}
